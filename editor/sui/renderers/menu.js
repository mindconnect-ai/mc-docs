...
export const MENU_STATES = ["expanded", "rail", "hidden"];
const STORAGE_PREFIX = "sui-menu:";
export function renderMenu(node, r) {
    ...
    const title = node.title
        ...
        : "";
    const nav = `<nav class="${cls(`sui-menu sui-menu--${state} sui-menu--${mode} sui-menu--${side}`, node)}" id="${id}" data-sui="menu" data-menu-state="${state}">
...
export function renderMenuItem(node, r) {
    ...
    return `<li class="sui-menu-item${activeCls}" id="${id}" data-id="${id}" role="none">
        <a class="sui-menu-link" href="${href}" ${nav}${current} role="menuitem">${icon}${label}${badge}</a>${tip}
    </li>`;
}
// ── Client-side state machine (used by the event bus + restore-on-load) ──────
/** Next state in the expanded → rail → hidden → expanded cycle. */
export function cycleMenuState(current) {
 ...
 */
export function nextMenuState(menu) {
...
export function menuStateOf(menu) {
    const s = menu.dataset.menuState;
 ...
 */
export function applyMenuState(menu, state, persist = true) {
    for (const s of MENU_STATES)
        menu.classList.toggle(`sui-menu--${s}`, s === state);
    ...
    if (persist && menu.id) {
        try {
            localStorage.setItem(STORAGE_PREFIX + menu.id, state);
        }
        catch { /* ignore */ }
    }
}
 ...
 */
export function restoreMenuState(root = document) {
    root.querySelectorAll(".sui-menu[id]").forEach(menu => {
        let stored = null;
        try {
            stored = localStorage.getItem(STORAGE_PREFIX + menu.id);
        }
        catch { /* ignore */ }
        if (stored && MENU_STATES.includes(stored)) {
            applyMenuState(menu, stored, false);
            return;
        }

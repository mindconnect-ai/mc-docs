 ...
 */
export function renderMenuButton(node, r) {
    ...
    return `<details class="${cls(`sui-menu-button sui-menu-button--${variant} sui-menu-button--align-${align}`, node)}" id="${id}" data-sui="menu-button">
...
function renderMenuButtonItem(node) {
    if (node.divider) {
    ...
    if (node.onClick) {
        return `<button type="button" class="sui-menu-button-item${dangerCls}" id="${id}" data-id="${id}" role="menuitem" data-trigger='${encodeTrigger(node.onClick)}'>${icon}${label}${badge}</button>`;
    }
 ...
 */
export function wireMenuButtons(_root = document) {
...
function onDocumentClick(e) {
    ...
    if (!target)
        return;
    const trigger = target.closest(".sui-menu-button-trigger");
    if (trigger) {
        const details = trigger.closest("details.sui-menu-button");
        if (details) {
            e.preventDefault(); // suppress the native <details> toggle; we drive it
            if (details.open)
                closeMenuButton(details);
            else {
                closeAllMenuButtons();
    ...
    const groupHeader = target.closest(".sui-menu-button-item--group");
    if (groupHeader) {
        const group = groupHeader.closest(".sui-menu-button-group");
        if (group) {
    ...
    const item = target.closest(".sui-menu-button-item");
    if (item) {
        const details = item.closest("details.sui-menu-button");
        if (details)
            closeMenuButton(details);
        return;
    }
    // A click anywhere outside a menu-button closes any that are open.
    if (!target.closest(".sui-menu-button"))
        closeAllMenuButtons();
}
function onDocumentKeydown(e) {
    if (e.key !== "Escape")
        return;
    const open = document.querySelector("details.sui-menu-button[open]");
    if (!open)
        return;
    closeAllMenuButtons();
    open.querySelector(".sui-menu-button-trigger")?.focus();
}
function closeAllMenuButtons() {
    document.querySelectorAll("details.sui-menu-button[open]").forEach(closeMenuButton);
}
function closeMenuButton(details) {
    details.open = false;
    details.querySelector(".sui-menu-button-trigger")?.setAttribute("aria-expanded", "false");
    // Collapse any expanded submenus so the menu reopens in its initial shape.
    details.querySelectorAll(".sui-menu-button-group.is-open").forEach(g => {
        g.classList.remove("is-open");
        g.querySelector(".sui-menu-button-item--group")?.setAttribute("aria-expanded", "false");
    });
    ...
    const pop = details.querySelector(".sui-menu-button-popover");
    if (pop)
        pop.removeAttribute("style");
}
function openMenuButton(details) {
    details.open = true;
    const trigger = details.querySelector(".sui-menu-button-trigger");
 ...
 */
function positionPopover(trigger, pop, alignStart) {
